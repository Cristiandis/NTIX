...
; Chocolatey — downloaded to a temp file and inspected before execution.
Filename: "powershell.exe"; \
Parameters: "-NoProfile -ExecutionPolicy Bypass -Command ""if (Get-Command choco -ErrorAction SilentlyContinue) {{ Write-Host 'Chocolatey already installed.'; exit 0 }}; $ErrorActionPreference='Stop'; $p = Join-Path $env:TEMP 'choco-install.ps1'; Invoke-WebRequest -UseBasicParsing -Uri 'https://community.chocolatey.org/install.ps1' -OutFile $p; & $p"""; \
...
; Scoop — same pattern.
Filename: "powershell.exe"; \
Parameters: "-NoProfile -ExecutionPolicy Bypass -Command ""if (Get-Command scoop -ErrorAction SilentlyContinue) {{ Write-Host 'Scoop already installed.'; exit 0 }}; $ErrorActionPreference='Stop'; $p = Join-Path $env:TEMP 'scoop-install.ps1'; Invoke-WebRequest -UseBasicParsing -Uri 'https://get.scoop.sh' -OutFile $p; & $p"""; \
